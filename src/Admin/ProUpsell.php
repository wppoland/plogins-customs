...
final class ProUpsell
{
    ...
    {
        if (! $this->sellable()) {
            return $this->isPolish() ? __('Wkrótce', 'customs') : __('Coming soon', 'customs');
        }
        $d = $this->data();
        if ($this->isPolish() && ! empty($d['price_pln'])) {
            /* translators: %d: yearly price in PLN */
            return sprintf(__('od %d zł/rok', 'customs'), (int) $d['price_pln']);
        }
        if (! empty($d['price_from'])) {
            $cur = ($d['currency'] ?? 'EUR') === 'EUR' ? '€' : (string) $d['currency'] . ' ';
            /* translators: 1: currency symbol, 2: yearly price */
            return sprintf(__('from %1$s%2$d/yr', 'customs'), $cur, (int) $d['price_from']);
        }
        return '';
    }
    ...
    private function ctaLabel(): string
    {
        return $this->sellable()
            ? __('Upgrade to PRO', 'customs')
            : ($this->isPolish() ? __('Powiadom mnie', 'customs') : __('Get notified', 'customs'));
    }
    ...
    public function handleDismiss(): void
    {
        if (! current_user_can('manage_woocommerce')) {
            wp_die(esc_html__('Permission denied.', 'customs'));
        }
    ...
    public function banner(): void
    {
        ...
        <div class="customs-pro-banner" role="note">
            ...
            <p class="customs-pro-banner__text">
                ...
                printf(esc_html__('Do more with %s', 'customs'), esc_html($name)); ?></strong>
            ...
            <a class="customs-pro-banner__dismiss" href="<?php echo esc_url($this->dismissUrl()); ?>" aria-label="<?php esc_attr_e('Dismiss this notice', 'customs'); ?>">&times;</a>
        </div>
        <?php
    }
    ...
    public function aside(): void
    {
        ...
        <aside class="customs-card customs-pro-aside" aria-labelledby="customs-pro-aside-h">
            ...
            <h2 id="customs-pro-aside-h" class="customs-pro-aside__heading"><?php esc_html_e('Unlock every PRO feature', 'customs'); ?></h2>
            ...
            <?php if ($price !== '') : ?>
                <p class="customs-pro-aside__price"><?php echo esc_html($price); ?><?php if ($this->sellable()) : ?> · <?php esc_html_e('one licence, every PRO feature', 'customs'); ?><?php endif; ?></p>
            <?php endif; ?>
        </aside>
        <?php
    }
    ...
    public function cards(): void
    {
        ...
        <section class="customs-pro-cards" aria-labelledby="customs-pro-cards-h">
            <h2 id="customs-pro-cards-h" class="customs-pro-cards__title">
                <?php
                /* translators: %s: PRO edition name */
                printf(esc_html__('What %s adds', 'customs'), esc_html($name)); ?>
            </h2>
